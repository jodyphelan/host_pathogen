...
subprocess.call("cat `ls %s.temp*` | pigz -p 20 -c > %s.results.gz" %(outfile,outfile),shell=True)
subprocess.call("cat %s.combin* | awk '{sum += $1} END {print sum}' > %s.numtests.txt" % (outfile,outfile),shell=True)
